/**
 * Tokenization + lightweight stemming for the retrieval engine.
 *
 * The retrieval engine operates on file *metadata* (path, name, folder) so
 * we only need a tiny normaliser — no full NLP, no models. The point is
 * that "auth", "Auth", "authentication", and "auth-service" should all
 * share a common base token ("auth") so question ↔ file matching is robust.
 *
 ...
 *     -> ["auth", "work"]
 *
 * This is what the retrieval engine matches against — a small, high-signal
